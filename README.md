...
Algorithm for fuzzy matching is much more comprehensive than fzf's algorithm it is **typo-resistant** and we provide a query language with additional constraint parsing for prefiltering e.g. "*.rs !/ shcema" is a perfectly valid query for fff, but fzf wouldn't find anything even for a single typo in "shcema".
